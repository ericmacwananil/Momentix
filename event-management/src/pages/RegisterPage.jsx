...
export default function RegisterPage() {
  ...
  return (
    // ✅ min-h instead of h, removed overflow-hidden — form was getting clipped on small phones
    <div className="min-h-[calc(100vh-64px)] bg-gradient-to-br from-orange-50 via-white to-orange-50 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-sm p-6 bg-white shadow-lg rounded-2xl sm:p-8">
        ...
        <div className="flex items-center justify-center gap-2.5 mb-5">
          <div className="inline-flex items-center justify-center flex-shrink-0 w-10 h-10 text-white shadow-md rounded-xl bg-gradient-to-br from-orange-500 via-amber-400 to-pink-500">
          ...
          <div>
            <h1 className="text-base font-bold leading-none text-gray-800 sm:text-lg">Create Account</h1>
            <p className="text-gray-400 text-xs leading-none mt-0.5">Join Momentix</p>
          </div>
        </div>

        {/* Role Selection */}
        <div className="mb-4">
          {/* ✅ Bumped label from text-[11px] to text-xs */}
          <label className="block mb-2 text-xs font-semibold text-gray-600">Select Role</label>
          <div className="grid grid-cols-3 gap-2">
            {ROLE_OPTIONS.map((role) => (
              <label key={role.id} className="cursor-pointer">
                ...
                <div className={`rounded-lg border-2 py-2 text-center transition ${selectedRole === role.id ? "border-orange-500 bg-orange-50 text-orange-700" : "border-gray-200 text-gray-500 hover:border-orange-200"}`}>
          ...
          {selectedRole === "admin" && (
            <p className="text-xs text-gray-500 bg-blue-50 rounded-lg px-2.5 py-1.5 mt-2">
        ...
        {error && (
          <div className="px-3 py-2 mb-3 text-xs text-red-600 border border-red-200 rounded-lg bg-red-50 sm:text-sm">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {[
            ...
            <div key={field.key}>
              <label className="block mb-1 text-xs font-semibold text-gray-600">{field.label}</label>
              <input
                type={field.type}
                required
          ...
          <button type="submit"
            className="w-full py-2.5 mt-1 text-sm font-semibold text-white transition bg-orange-500 rounded-lg hover:bg-orange-600 active:scale-95">
            Create Account
          </button>
        </form>
